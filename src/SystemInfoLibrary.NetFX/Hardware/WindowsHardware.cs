    ...
    internal class WindowsHardwareInfo : HardwareInfo
    {
        ...
        public override IList<CPUInfo> CPUs
        {
            get
            {
                if (_CPUs == null)
                {
                    if (int.TryParse(Utils.Win32_ComputerSystem["NumberOfProcessors"], 0, CultureInfo.InvariantCulture, out var value))
                    {
                        if (value == 1)
                    ...
                    if (_CPUs == null)
                        throw new Exception("Could not get 'NumberOfProcessors' from 'Win32_ComputerSystem'");
                }
